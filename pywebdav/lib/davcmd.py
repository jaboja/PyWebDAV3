...
def deltree(dc, url, exclude={}):
    """ delete a tree of resources

    dc  -- dataclass to use
    url -- root url to delete
    exclude -- an optional list of url:error_code pairs which should not
           be deleted.

    returns dict of url:error_code tuples from which
    ...
    tlist = create_treelist(dc, url)
    result = {}

    for i in range(len(tlist), 0, -1):
        problem_urls = list(result.keys())
        element = tlist[i - 1]

        # test here, if an element is a prefix of an url which
...
def delone(dc, url):
    ...
    if dc.is_collection(url):
        return dc.rmcol(url)  # should be empty
    else:
        return dc.rm(url)
...
def copytree(dc, src, dst, overwrite=None):
    ...
    dc  -- dataclass to use
    src -- src url from where to copy
    dst -- dst url
    overwrite -- if True then delete dst url before
    ...
    for element in tlist:
        ...
        # able to copy in problem_urls which is the prefix
        ...
        dst_url = urllib.parse.urlunparse(dst_parsed._replace(
            path=os.path.join(dst_parsed.path, element_path_rel)))

        # now copy stuff
        try:
            copy(dc, element, dst_url)
        except DAV_Error as error:
            (ec, dd) = error.args
            result[element] = ec
