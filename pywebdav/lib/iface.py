...
class dav_interface:
    ...
    def get_prop2(self, url, ns, pname):
        """ return the value of a property

        """
        if ns.lower() == "dav:":
            return self.get_dav(url, pname)

        raise DAV_NotFound

    def get_prop(self, url, ns, propname):
        """ return the value of a given property

        url        -- url of the object to get the property of
    ...
    def get_data(self, url, byte_range=None):
    ...
    def put(self, url, data, content_type=None):
        ...
        return the location url or raise an exception
        """
    ...
    # LOCKing information
    #
    def _get_dav_supportedlock(self, url):

        txt = (
            '<main xmlns:D="http://dummy/d" xmlns:ns1="http://webdav.de/ns1"><D:lockentry>\n'
    ...
    def _get_dav_lockdiscovery(self, url):
        lcm = LockManager()
        if lcm._l_isLocked(url):
            lock = lcm._l_getLockForUrl(url)
    ...
    ad 1) The first approach can be used when we know that no error can 
          ...
          it in a dict of the form {url: error_code, ...}
