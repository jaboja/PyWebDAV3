...
class Test(unittest.TestCase):
    ...
    def test_run_litmus(self):
        ...
        try:
            ...
            davserver_cmd = [sys.executable, os.path.join(
                testdir, '..', 'pywebdav', 'server', 'server.py'), '-D',
                             self.rundir, '-u', user, '-p', password,
                             '-H', 'localhost', '--port', str(port)]
            self.davserver_proc = subprocess.Popen(davserver_cmd, env=env)
            ...
            try:
                ret = run(["make", "URL=http://localhost:%d" % port,
                           'CREDS="%s %s"' % (user, password), "check"],
                          cwd=self.litmus_dist, capture_output=True)
                results = ret.stdout
    ...
    def test_run_litmus_noauth(self):
        ...
        try:
            ...
            print('Running litmus')
            try:
                ret = run(["make", "URL=http://localhost:%d" % port, "check"],
                          cwd=self.litmus_dist, capture_output=True)
                results = ret.stdout
